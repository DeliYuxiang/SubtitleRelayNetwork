...
# ── Generate wrangler config from env vars (never mutates the committed file) ──
# Must live inside worker/ so wrangler resolves src/index.ts correctly.
WRANGLER_TMP="$REPO_ROOT/worker/wrangler.production.jsonc"
trap 'rm -f "$WRANGLER_TMP"' EXIT
SRN_POW_SECRET=$(openssl rand -hex 16) \
  "$REPO_ROOT/.github/workflows/scripts/generate_wrangler_config.sh" "$WRANGLER_TMP"
