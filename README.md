...
本项目提供标准的 RESTful 接口，并附带自动生成的 Swagger 文档。

| 路径 | 方法 | 功能 | 参数 / 说明 |
| :--- | :--- | :--- | :--- |
| `/` | `GET` | **门户首页** | 提供基于 Petite-Vue 的单页搜索与下载应用。 |
| `/v1/health` | `GET` | **健康徽章** | Shields.io 兼容接口，实时显示 `Online` 状态与索引总数。 |
| `/v1/events` | `GET` | **查询事件** | `tmdb`, `season`, `ep`, `language`, `archive_md5` (可选)。 |
| `/v1/events` | `POST` | **发布事件** | 需要 Ed25519 签名认证。详见下文。 |
| `/v1/events/:id/content` | `GET` | **内容下载** | 这里的 `id` 为事件的哈希。直接从 R2 流式输出。 |
| `/ui` | `GET` | **Swagger UI** | 交互式 API 文档页面。 |
| `/doc` | `GET` | **OpenAPI JSON** | 导出的标准 OpenAPI v3 Schema。 |

### 数据库维护 (D1 Schema)
- **本地定义**：见 [worker/schema.sql](./worker/schema.sql)。
- **在线导出**：使用 `npx wrangler d1 export <db-name> --remote --output ./backup.sql`。

### 签名认证 (Post Event)
发布新字幕时，必须在 Header 中携带：
- `X-SRN-PubKey`: 发布者的 Ed25519 公钥。
- `X-SRN-Signature`: 对事件 JSON 载荷（不含文件部分）的签名。

中继节点会校验：
1. 签名有效性。
2. 文件大小是否在 5MB 以内。
3. `archive_md5` 是否与实际上传的文件内容一致。
